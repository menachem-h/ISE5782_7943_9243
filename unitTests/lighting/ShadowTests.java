...
public class ShadowTests {
	...
	void sphereTriangleHelper(String pictName, Triangle triangle, Point spotLocation) {
		lights.clear();
		lights.add(new SpotLight(new Color(400, 240, 0), spotLocation, new Vector(1, 1, -3)) //
				.setkL(1E-5).setkQ(1.5E-7));
		 Scene scene = new Scene.SceneBuilder("Test scene").
		setGeometries(new Geometries(sphere,triangle.setEmission(new Color(BLUE)).setMaterial(trMaterial))).
		setLights(lights)
				 .build();
		...
		Camera camera = new Camera.CameraBuilder(new Point(0, 0, 1000), new Vector(0, 0, -1), new Vector(0, 1, 0)) //
				...
				setImageWriter(new ImageWriter(pictName, 400, 400)).build();
	...
	public void trianglesSphere() {
		...
		lights.clear();
		lights.add(	new SpotLight(new Color(700, 400, 400), new Point(40, 40, 115), new Vector(-1, -1, -4)) //
				.setkL(4E-4).setkQ(2E-5));
		Scene scene = new Scene.SceneBuilder("Test scene").
				setGeometries(new Geometries(new Triangle(new Point(-150, -150, -115), new Point(150, -150, -135), new Point(75, 75, -150)) //
		...
		Camera camera = new Camera.CameraBuilder(new Point(0, 0, 1000), new Vector(0, 0, -1), new Vector(0, 1, 0)) //
				.setVPSize(200, 200).setVPDistance(1000) //
				.setRayTracer(new RayTracerBasic(scene)).
				setImageWriter(new ImageWriter("shadowTrianglesSphere", 600, 600)).build();
